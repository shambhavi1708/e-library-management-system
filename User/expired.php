...
<br><br><br><br>
        ...
        $sql="SELECT student_register.username,books.bid,name,authors,edition,issue,issue_book.approve,issue_book.return FROM student_register inner join issue_book ON student_register.username=issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve ='$ret' and issue_book.username ='$_SESSION[login_student]'  ORDER BY `issue_book`.`return` DESC";
        $res=mysqli_query($db,$sql);

        }
        else if(isset($_POST['submit3']))
        {
        $sql="SELECT student_register.username,books.bid,name,authors,edition,issue,issue_book.approve,issue_book.return FROM student_register inner join issue_book ON student_register.username=issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve ='$exp' and issue_book.username ='$_SESSION[login_student]' ORDER BY `issue_book`.`return` DESC";
        $res=mysqli_query($db,$sql);
        }
        else
        {
        $sql="SELECT student_register.username,books.bid,name,authors,edition,issue,issue_book.approve,issue_book.return FROM student_register inner join issue_book ON student_register.username=issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve !='' and issue_book.approve !='Yes'  and issue_book.username ='$_SESSION[login_student]' ORDER BY `issue_book`.`return` DESC";
        $res=mysqli_query($db,$sql);
        }
