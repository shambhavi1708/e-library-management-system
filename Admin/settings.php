<?php
include "connection.php"; // This should contain your database connection details
session_start();
$servername = "localhost";
$username = "root";
$password = "";
$dbname = "vault_library_management"; // Make sure to use the correct database name
...
$email_update_msg = '';
$password_update_msg = '';
$account_delete_msg = ''; // Initialize this variable to avoid undefined variable warning

// Fetch the admin's current email from the database
$stmt = $conn->prepare("SELECT email FROM admin_register WHERE username = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['email'])) {
        ...
        $stmt = $conn->prepare("UPDATE admin_register SET email = ? WHERE username = ?");
    ...
    if (isset($_POST['newPassword']) && isset($_POST['currentPassword'])) {
        ...
        $stmt = $conn->prepare("SELECT password FROM admin_register WHERE username = ?");
        $stmt->bind_param("s", $login_user);
        $stmt->execute();
        ...
        if ($current_password === $stored_password) {
            ...
            if ($new_password === $confirm_new_password) {
                ...
                $stmt = $conn->prepare("UPDATE admin_register SET password = ? WHERE username = ?");
    ...
    // If the account deletion form is submitted
    if (isset($_POST['deleteAccount'])) {
        // Prepare and execute delete statement
        $stmt = $conn->prepare("DELETE FROM admin_register WHERE username = ?");
        $stmt->bind_param("s", $login_user);
